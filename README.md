<div align="center">

# 👋 Hi, I'm Shayan Bhattacharya

### 🚀 Application Support Analyst | DevOps Engineer | Cloud Enthusiast | AI Agent Builder

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=24&pause=1000&center=true&vCenter=true&width=900&lines=Application+Support+Engineer;DevOps+Engineer;AWS+Cloud+Enthusiast;Docker+%7C+Kubernetes+%7C+Terraform;AI+Agents+%7C+Automation+Builder;Always+Learning+Always+Building" />

</div>

---

# 💫 About Me

🔹 4+ Years of Experience in the IT Industry

🔹 Application Support & DevOps Professional

🔹 Experience in SaaS Applications, APIs, Monitoring & Incident Management

🔹 Passionate about Cloud Computing, DevOps, AI Agents & Automation

🔹 Building AI Solutions using Claude AI, Ollama, Gemini, AWS Bedrock & N8N

🔹 Currently Learning:
- Kubernetes Advanced Concepts
- AI Agents & RAG Systems
- AWS AI Services
- DevSecOps

---

# 🛠️ Tech Stack

## Cloud & DevOps

<p align="left">
<img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,terraform,jenkins,linux,bash,git,github" />
</p>

## Programming

<p align="left">
<img src="https://skillicons.dev/icons?i=python,mysql" />
</p>

## Monitoring & Observability

<p align="left">
<img src="https://skillicons.dev/icons?i=grafana,prometheus" />
</p>

## AI & Automation

- Claude AI
- Ollama
- AWS Bedrock
- Gemini AI
- N8N
- AI Agents
- RAG Applications
- Prompt Engineering

---

# 🚀 Featured Projects

## 🤖 AI Chatbot for GPS Tracking Software

AI-powered assistant using:

- Ollama
- RAG
- Knowledge Base Search
- Vector Database

### Tech Used

Python | Ollama | AI Agents

---

## 🔐 SSL Automation Platform

Automated SSL Deployment Solution

### Features

- Jenkins Pipeline
- Nginx SSL Deployment
- Tomcat SSL Deployment
- SSL Monitoring

### Tech Used

Jenkins | Bash | Linux

---

## ☸️ Kubernetes Zero Downtime Deployment

Deployment Strategies:

- Rolling Update
- Blue Green
- Canary Deployment

### Tech Used

Kubernetes | Docker | YAML

---

## 📲 AI Agent Task Manager

Workflow Automation Platform

### Features

- Google Forms
- Airtable
- WhatsApp Alerts
- HubSpot Integration

### Tech Used

N8N | AI Agents | APIs

---

# 📜 Certifications & Learning

✅ DevOps

✅ AWS Cloud

✅ AI Agents

✅ Google AI Agent Crash Course

✅ Kubernetes Learning Path

---

# 📈 GitHub Statistics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&theme=tokyonight&hide_border=true"/>

</div>

---

# 🔥 GitHub Streak

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_GITHUB_USERNAME&theme=tokyonight&hide_border=true"/>

</div>

---

# 📊 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=YOUR_GITHUB_USERNAME&theme=tokyo-night"/>

</div>

---

# 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=YOUR_GITHUB_USERNAME&theme=tokyonight&row=1&column=7"/>

</div>

---

# 🌱 Current Focus

```yaml
Cloud:
  - AWS
  - Kubernetes
  - Terraform

DevOps:
  - CI/CD
  - Automation
  - Infrastructure as Code

AI:
  - AI Agents
  - Ollama
  - Claude AI
  - AWS Bedrock

Security:
  - DevSecOps
  - OWASP
  - Snyk
```

---

# 📅 2026 Goals

- Build Production AI Agent
- Complete Advanced Kubernetes
- Earn AWS Certification
- Contribute to Open Source
- Launch AI Automation Projects
- Grow Neo Smart Academy

---

# 🌐 Connect With Me

<p align="left">
<a href="https://linkedin.com/in/YOUR_LINKEDIN_PROFILE">
<img src="https://skillicons.dev/icons?i=linkedin" />
</a>

<a href="mailto:yourmail@example.com">
<img src="https://skillicons.dev/icons?i=gmail" />
</a>
</p>

---

<div align="center">

### ⭐ Building DevOps, Cloud & AI Solutions One Project at a Time

![Profile Views](https://komarev.com/ghpvc/?username=YOUR_GITHUB_USERNAME&color=blue)

</div>
